...
\begin{solution}
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Repeat the procedure described in the book to get a sequence of nested intervals. Apply NIP. It's not difficult to conclude that the intersection is 0.
  \end{enumerate}
\end{solution}
...
\begin{solution}
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Similar to part (a), consider the function $g(x) = f(x) - f(x + \frac{1}{n})$. Then the required condition is equivalent to the existence of $g(x) = 0$.
    
    Assume $n=3$ and consider $g(0), g(\frac{1}{3}), $ and $g(\frac{2}{3})$. If any of them are zero then we're done. Otherwise, like part (a) we'll prove that there exist
    $i < j$ such that $g(i)$ and $g(j)$ have different signs. Then we'll apply IVT to conclude the existence of $g(x)=0$ for some $x$.

    Is it possible for all three $g$'s to be positive? Suppose $g(0)$ and $g(\frac{1}{3})$ are positive, then we can conclude that $f(0) > f(\frac{1}{3}) > f(\frac{2}{3})$.
    But $f(0)=f(1)$, so we must have $f(1) > f(\frac{2}{3}) \implies g(\frac{2}{3})$ is negative.

    A very similar argument can be used to prove that we can't have all three $g$'s negative, and it's also easy to generalize to arbitrary $n$.

    Equivalently, we can express the same argument with telescoping sum. Consider 
    \[\sum_{k=0}^{n-1}g\left(\frac{k}{n}\right) = \sum_{k=0}^{n-1}\left[ f\left(\frac{k}{n}\right) - f\left(\frac{k+1}{n}\right) \right]\]

    This sum telescopes to $f(0)-f(1)=0$. Since the sum is zero, we can't have all terms positive or all terms negative.
    ...
    \[
      ...
      \begin{cases}
        x & x \in [0, \frac{4}{5}]\\
        \frac{4}{5} + -4(x - \frac{4}{5}) = 4-4x & x \in (\frac{4}{5}, 1]
      \end{cases}
    \]
    Basically, a line with slope 1 from 0 to $\frac{4}{5}$, then a steeper descent with slope -4 from $\frac{4}{5}$ to 1.

    For $x \in [0, \frac{2}{5}]$, clearly $f(x) \neq f(x+\frac{2}{5})$ since $f(x)=x$ and $f(x+\frac{2}{5})=x+\frac{2}{5}$.

    Now to prove the same for $x \in (\frac{2}{5}, \frac{3}{5}]$. We have $f(x) = x$ and $f(x+\frac{2}{5}) = 4-4(x+\frac{2}{5}) = \frac{12}{5}-4x$. If $f(x)=f(x+\frac{2}{5})$ then
    we need an $x$ such that $x = \frac{12}{5}-4x \implies 5x = \frac{12}{5} \implies x = \frac{12}{25}$. Eh... this doesn't work after all.
  \end{enumerate} 
\end{solution}
...
\begin{solution}
  Stuck with this for quite some time and finally got a hint from chatgpt: consider the auxiliary function $g(x)=f(x)-x$. Geometrically, we're asking if $f$ intersects the line $y=x$, and we define
  $g$ to be the vertical distance between them.
