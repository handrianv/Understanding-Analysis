...
\begin{enumerate}[label=(\alph*)]
  \item $g_{a}$ is differentiable on \textbf{R} but such that $g'_{a}$ is unbounded on $[0, 1]$.
  \item  $g_{a}$ is differentiable on \textbf{R} with $g'_{a}$ continuous but not differentiable at zero.
  \item $g_{a}$ is differentiable on \textbf{R} and $g'_{a}$ is differentiable on \textbf{R}, but such that $g''_{a}$ is not continuous at zero.
\end{enumerate}

\begin{solution}
  We're going to use the fact that the derivative of $x^a$ is $ax^{a-1}$ for $a \in \textbf{R}$, even though the book has only proved it for $a \in \textbf{N}$.
  \begin{enumerate}[label=(\alph*)]
    \item
    \[
      g'_{a}(x) = 
      ...
      \end{cases}
    \]
    For $a < 1$, $g'(0)$ is not defined because the $x^{a-1}$ makes it unbounded. For $a=1$, $g'(0)=\lim_{x \to 0}\sin(1/x)$, which is also not defined (Example 4.2.6).
    For $a \geq 2$, $g_{a}'(x)$ is bounded in $[0, 1]$.
  
    The answer is then $1 < a < 2$, for which $g_{a}'(x)$ is defined everywhere and, by the $x^{a-2}\cos(1/x)$ term is unbounded as 
    $x \to 0$. But we need to be a little careful in choosing $a$ to avoid even roots
    like $\sqrt{-1}$ which is not defined in \textbf{R}. We can choose $a=\frac{5}{3}$, which gives

    \[
      g'_{a}(x) = 
      \begin{cases}
        \frac{5}{3}x^{2/3}\sin(1/x) - x^{-\frac{1}{3}}\cos(1/x) & \text{ if } x \neq 0\\
        0 & \text{ if } x = 0
      \end{cases}
    \]
   ...
   \[
      g'_{a}(x) = 
      \begin{cases}
        3x^{2} \sin(1/x) - x \cos(1/x) & \text{ if } x \neq 0\\
        0 & \text{ if } x = 0
      \end{cases}
   \]

   Which is continuous at zero. But the second derivative at zero
   \[
     g''_{a}(0) = \lim_{x \to 0}\frac{g'(x)-g'(0)}{x} = \lim_{x \to 0} 3x \sin(1/x) - \cos(1/x)
   \]
   is undefined, since $3x \sin(1/x)$ tends to zero but $\cos(1/x)$ oscillates between -1 and 1.
  
  \item Let $a=4$. Then
   \[
      g'_{a}(x) = 
      \begin{cases}
        4x^{3} \sin(1/x) - x^{2} \cos(1/x) & \text{ if } x \neq 0\\
        0 & \text{ if } x = 0
      \end{cases}
   \]
   ...
   \[
      g''_{a}(x) = 
      \begin{cases}
        12x^{2} \sin(1/x) - 6x \cos(1/x) - \sin(1/x) & \text{ if } x \neq 0\\
        0 & \text{ if } x = 0
      \end{cases}
   \]
   Which is discontinuous at zero because in the equation for $x \neq 0$, the first two terms tend to zero while $\sin(1/x)$ oscillates between -1 and 1.
  \end{enumerate}
\end{solution}
...
%------------------------- Problem 10 -------------------------
\miquestion Recall that a function $f: (a, b) \to \textbf{R}$ is increasing on $(a, b)$ if $f(x) \leq f(y)$ whenever $x < y$ in $(a, b)$.
A familiar mantra from calculus is that a differentiable function is increasing if its derivative is positive, but this statement requires some sharpening
in order to be completely accurate. Show that the function
\[
  g(x) = 
  \begin{cases}
  ...
  \end{cases}
\]
is differentiable on \textbf{R} and satisfies $g'(0) > 0$. Now, prove that $g$ is \textit{not} increasing over any open interval containing 0.

In the next section we will see that
$f$ is indeed increasing on $(a, b)$ if and only if $f'(x) \geq 0$ for all $x \in (a, b)$.

\begin{solution}
  Heh, turns out this is the solution to the previous problem... Anyway, the derivative is
  \[
    g'(x) = 
    \begin{cases}
      \frac{1}{2} + 2x \sin(1/x) - \cos(1/x) & \text{ if } x \neq 0\\
      \frac{1}{2} & \text{ if } x=0\\
    \end{cases}
  \]

  So $g$ is differentiable on \textbf{R} and $g'(0) > 0$ as required.

  To prove that $g$ is not increasing over any open interval containing 0, choose an $x$ such that $\cos(1/x)=1$, hence $\sin(1/x)=0$ and $g'(x)=-\frac{1}{2}$.
  Since the derivative is negative, $g$ cannot be increasing.
\end{solution}

%------------------------- Problem 11 -------------------------
\miquestion Assume that $g$ is differentiable on $[a, b]$ and satisfies $g'(a) < 0 < g'(b)$.
\begin{enumerate}[label=(\alph*)]
  \item Show that there exists a point $x \in (a, b)$ where $g(a) > g(x)$ and a point $y \in (a, b)$ where $g(y) < g(b)$.
  \item Now complete the proof of Darboux Theorem started earlier.
\end{enumerate}

\begin{solution}
  \begin{enumerate}[label=(\alph*)]
    \item The derivative at $a$ is
    \[\lim_{h \to 0}\frac{g(a+h)-g(a)}{h} < 0\]
    The denominator is positive. For the result to be negative, it must be that  $g(a+h) < g(a)$ for small enough $h$.

    Similarly, the derivative at $b$ is
    \[\lim_{h \to 0}\frac{g(b)-g(b-h)}{h} > 0\]
    The denominator is positive. For the result to be positive, it must be that  $g(b) > g(b-h)$ for small enough $h$.

    \item To recap the book's partial proof of the theorem: let $g(x)=f(x)-\alpha x$ on $[a, b]$. Notice that $g$ is differentiable
    on $[a, b]$ with $g'(x)=f'(x)-\alpha$. In terms of $g$, the theorem's hypothesis states that $g'(a) < 0 < g'(b)$ and we need to show
    that $g'(c)=0$ for some $c \in [a, b]$.

    Now to complete the proof. $g$ differentiable on $[a, b]$ implies it's continuous on $[a, b]$. By the Extreme Value Theorem, $g$ attains a minimum value at a
    point $c$. By part(a), $c \neq a$ and $c \neq b$. By the Interior Extremum Theorem, $g'(c) = 0$.
  \end{enumerate}
\end{solution}

%------------------------- Problem 12 -------------------------
\miquestion \textbf{(Inverse functions)}. If $f: [a, b] \to \textbf{R}$ is one-to-one, then there exists an inverse function $f^{-1}$ defined on the range of $f$ given by
$f^{-1}(y)=x$ where $y=f(x)$. In Exercise 4.5.8 we saw that if $f$ is continuous on $[a, b]$, then $f^{-1}$ is continuous on its domain. Let's add the assumption that $f$
is differentiable on $[a, b]$ with $f'(x) \neq 0$ for all $x \in [a, b]$. Show $f^{-1}$ is differentiable with
\[(f^{-1})'(y)=\frac{1}{f'(x)}\]

\begin{solution}
  Let $c \in [a, b]$. Then the derivative of $f^{-1}$ at $f(c)$ is
  \begin{align*}
    (f^{-1})'(f(c)) &= \lim_{y \to f(c)}\frac{f^{-1}(y)-f^{-1}(f(c))}{y-f(c)}\\
    &= \lim_{y \to f(c)}\frac{x-c}{f(x)-f(c)}
  \end{align*}
  
  Since $f^{-1}$ is continuous on its domain, we can conclude that $y \to f(c) \implies f^{-1}(y) \to f^{-1}(f(c))$ or equivalently, $y \to f(c) \implies x \to c$. Applying
  this to the limit above
  \begin{align*}
    (f^{-1})'(f(c)) &=\lim_{y \to f(c)}\frac{x-c}{f(x)-f(c)}\\
    &= \lim_{x \to c}\frac{x-c}{f(x)-f(c)}\\
    &= \frac{1}{f'(c)}
  \end{align*}
  Note that in the second equality, $f(x)-f(c) \neq 0$ in the denominator because $f$ is one-to-one, hence $x \neq c \implies f(x) \neq f(c)$. In the last equality,
  $f'(c) \neq 0$ by the problem's assumption.
\end{solution}
